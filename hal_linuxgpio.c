...
static unsigned char  gpios[MAX_PIN] = {[0 ... MAX_PIN-1] = -1};
static unsigned char    dir[MAX_PIN] = {[0 ... MAX_PIN-1] =  1};
static          int gpiosfd[MAX_PIN] = {[0 ... MAX_PIN-1] = -1};

static int npins = 0;

MODULE_AUTHOR("Tran Ngoc Quan");
...
static void rw_port(void *arg, long period);
  ...
  for (n = 0; n < npins; n++)
    {
      if (dir[n])
        {
        	if (1 != write(gpiofd[n], *(port_data[n]) ? "1" : "0", 1)) {
...
static int parse_conf()
{
  int p, i;
  unsigned char npins;
  int invalid; /* flag indicate pin is invalid */
  #define BUFFER_MAX 3
  ...
  for (i = 0; i < MAX_PIN -1; i++)
    {
      ...
      snprintf(path, DIRECTION_MAX, "/sys/class/gpio/gpio%d/direction", p);
      if(!access(path, F_OK )){
        continue;
      }
      ...
      /*Export*/
	    ...
	    bytes_written = snprintf(buffer, BUFFER_MAX, "%d", p);
